    ...
    <div class="dislpay-arena">
      ...
      <div class="coordinates-header">
        ...
        <div class="form-group">
          <input type="text" class="form-control location-control my-1" v-model="location" @input="event => addressEdited = true"/>
          ...
          <button type="button" v-if="locationFound && !addressEdited" :disabled="loading" :class="{ disabled: loading }" class="btn btn-info mx-1"
            @click="toggleEditPosition">
            {{ btnEditPositionText }}
          </button>
          <p v-if="editPositionFlg">Move marker</p>
        </div>
        <SearchButton v-if="locationFound" :access-token="access_token" :current-location="center" @found-places="showMarkers" />
